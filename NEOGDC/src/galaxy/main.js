$(document).ready(function(){
    ...
    function initGalaxy(parsedData) {
        ...
        var instructableIds = _.keys(parsedData); //extract all keys and return an array
        ...
        _.each(window.Galaxy.Settings.categories,function(){
            ...
            // define attributes for vertex shader input
            attributes = {
                size: { type: 'f', value: [] },
                ca: { type: 'c', value: [] },
                alpha: { type: 'f', value: [] }
            ...
            // define attributes for fragement shader input
            uniforms = {
        ...
        _.each(instructableIds,function(id){
            //assign absolute position in canvas to each particle
            ...
            var ring = indexForCategory(parsedData[id].category); // six categories
            if (ring !== -1) {
                particleGeometries[ring].vertices.push(particle); // create another array in particleGeometries and push particle into it

                // according to views, each particle has its own attributes
                var appearance = Galaxy.Utilities.vertexAppearanceByViews(parsedData[id].views);
                shaderMaterialsArray[ring].attributes.size.value.push(appearance.size); // create array and push
        ...
        // create the particle system (each ring)
...
//            console.log('lightness ',lightness);
        ...
        // prepare secondary composer -> post processing
        ...
        // will change what we normally do for rendering from renderer.render() to composer.render()
